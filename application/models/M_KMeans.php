...
class M_KMeans extends CI_Model {
	...
	function __construct() {
		...
		$dataWarga = $this->db->get('wargas')->result_array();
		$this->data=[];
		$this->provinsi=[];
		foreach ($dataWarga as $key => $value) {
			$this->data[]=$value;
			$this->provinsi[]=$value['nama'];
		}
		...
		foreach ($dataWarga as $key => $value) {
			$dataForProcessing[]=[$value['is_pns'],$value['gaji']];
		}
		// jumlah cluster
		$this->cluster = 3;
		$this->cluster_lama = $this->cluster;
		$this->variable_x = 'PNS';
		$this->variable_y = 'Gaji';
	...
	function centroidBerubah($centroid,$iterasi){
		...
		for($i=0;$i<count($centroid_lama);$i++){
			if(isset($centroid_baru[$i])) {
				$centroidBaruCheck = $centroid_baru[$i];
			} else {
				$centroidBaruCheck = 0;
			}
			if($centroid_lama[$i]===$centroidBaruCheck){
				$jumlah_sama++;
			}
		}
	...
	function process() {
		...
		return array(
			...
			'variable_x' => $this->variable_x,
			'variable_y' => $this->variable_y,
			'provinsi' => $this->provinsi,
			'centroid' => $this->centroid,
			'data' => $this->dataForProcessing,
			'cluster' => $this->cluster_lama
		);
	}
}
?>
